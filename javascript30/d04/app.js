...
const years = inventors.reduce((total, inventor) => { return total + (inventor.passed - inventor.year); }, 0);
console.log(years);
...
// const links = [...category.querySelectorAll('a')];          // converting node list to array through es6
...
const alpha = people.sort((previous, next) => {
  const [aL, aF] = previous.split(', ');
  const [bL, bF] = previous.split(', ');
  return aL > bL ? 1 : -1;
});
console.log(alpha);
